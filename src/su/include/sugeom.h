...
/* 
 * These variables are declared as extern to avoid multiple definition
 * errors when this header is included by multiple source files.
 * The actual definitions should be in the source file that uses them.
 */
extern char textbuffer[120];      /* buffer for SPS records read and process */

extern int verbose;               /* flag to get advisory messages */
...
/*
 * Note: segy tr is NOT declared here.
 * Each SU program should define its own local segy tr variable.
 * Having segy tr in a header causes multiple definition errors
 * with MSVC and modern GCC (-fno-common).
 */
