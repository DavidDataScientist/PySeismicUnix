...
#ifdef _WIN32
/* Include Windows headers first, then our compatibility headers */
#include <io.h>
#include <direct.h>
#include "win32_compat/sys/stat.h"
#include "win32_compat/sys/sysmacros.h"
#else
#include <sys/stat.h>
...
FileType filestat(int fd)
{
...
#ifdef _WIN32
	if ((sfd.st_mode & S_IFMT) == S_IFSOCK) return SOCKET_FILE;
#else
	...
	/* Note: On Windows, tape detection will not work as	*/
	/* /dev/rmt0 and /dev/mt0 don't exist.			*/
#ifndef _WIN32
...
#else
	/* On Windows, /dev/null is typically NUL: */
	if (0 == stat("NUL", &sdn) &&
		sfd.st_rdev == sdn.st_rdev) return DISK;
#endif
...
#ifdef _WIN32
		case SOCKET_FILE:	return "SOCKET";
#else
