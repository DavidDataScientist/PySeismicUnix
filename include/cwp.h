...
#include <stdio.h>
#include <stddef.h>  /* Must be included early for size_t */
#include <stdlib.h>
#include <string.h>
#include <math.h>
#include <ctype.h>
...
#ifdef _WIN32
/* Prevent MSVC from including complex.h which might define complex as a macro */
/* Also prevent winsock.h from being included */
#ifndef WIN32_LEAN_AND_MEAN
#define WIN32_LEAN_AND_MEAN
#endif
/* Disable Windows min/max macros that conflict with CWP code */
#ifndef NOMINMAX
#define NOMINMAX
#endif
...
/* This must happen before we define our complex type */
#ifdef complex
#undef complex
#endif
/* Also prevent _Complex keyword from being used */
#ifdef _Complex
#undef _Complex
#endif
#endif
...
#ifdef _WIN32
/* Prevent Windows.h from including winsock.h which defines SOCKET */
#ifndef WIN32_LEAN_AND_MEAN
#define WIN32_LEAN_AND_MEAN
#endif
...
/* After including all headers, undefine complex macro if MSVC defined it */
#ifdef complex
#undef complex
#endif
#ifdef _Complex
...
/* This applies to CRAY, OVERRIDE_CWP_COMPLEX, and Windows/MSVC */
...
/* On Windows, undefine complex macro if MSVC defined it */
/* This must happen AFTER all standard headers are included */
#ifdef _WIN32
/* Force undefine complex macro - do this multiple times to be safe */
#ifdef complex
#undef complex
#endif
#ifdef _Complex
#undef _Complex
#endif
/* Also check after including win32_compat headers */
#endif

/* Define complex and dcomplex as aliases to cwp_complex and cwp_dcomplex */
/* This macro definition will override any previous complex macro */
#ifdef _WIN32
/* On Windows, ensure complex is not a macro before defining it */
#ifdef complex
#undef complex
#endif
#endif
...
/* Function name mappings (for CRAY/OVERRIDE compatibility) */
...
/* On Unix (non-CRAY, non-Windows), use standard definition */
#ifndef complex
typedef struct _complexStruct { /* complex number */
	float r,i;
} complex;
...
/* Define CSIZE after complex type is defined */
#ifndef CSIZE
#if defined(CRAY) || defined(OVERRIDE_CWP_COMPLEX) || defined(_WIN32)
/* Use cwp_complex directly for custom type implementations */
#define CSIZE sizeof(cwp_complex)
#else
...
/* CSIZE defined after complex type is defined - see below */
...
#if defined(CRAY) || defined(OVERRIDE_CWP_COMPLEX) || defined(_WIN32)
/* Use cwp_complex directly in function declarations to avoid macro expansion issues */
cwp_complex *alloc1complex (size_t n1);
cwp_complex *realloc1complex (cwp_complex *v, size_t n1);
cwp_complex **alloc2complex (size_t n1, size_t n2);
cwp_complex ***alloc3complex (size_t n1, size_t n2, size_t n3);
#else
...
#if defined(CRAY) || defined(OVERRIDE_CWP_COMPLEX) || defined(_WIN32)
void free1complex (cwp_complex *p);
void free2complex (cwp_complex **p);
void free3complex (cwp_complex ***p);
#else
...
#if defined(CRAY) || defined(OVERRIDE_CWP_COMPLEX) || defined(_WIN32)
cwp_complex cadd (cwp_complex a, cwp_complex b);
cwp_complex csub (cwp_complex a, cwp_complex b);
cwp_complex cmul (cwp_complex a, cwp_complex b);
cwp_complex cdiv (cwp_complex a, cwp_complex b);
float rcabs (cwp_complex z);
cwp_complex cmplx (float re, float im);
cwp_complex conjg (cwp_complex z);
cwp_complex cneg (cwp_complex z);
cwp_complex cinv (cwp_complex z);
cwp_complex cwp_csqrt (cwp_complex z);
cwp_complex cwp_cexp (cwp_complex z);
cwp_complex crmul (cwp_complex a, float x);

/* complex functions */
cwp_complex cipow(cwp_complex a, int p);
cwp_complex crpow(cwp_complex a, float p);
cwp_complex rcpow(float a, cwp_complex p);
cwp_complex ccpow (cwp_complex a, cwp_complex p);
cwp_complex cwp_ccos(cwp_complex a);
cwp_complex cwp_csin(cwp_complex a);
cwp_complex cwp_ccosh(cwp_complex a);
cwp_complex cwp_csinh(cwp_complex a);
cwp_complex cwp_cexp1(cwp_complex a);
cwp_complex cwp_clog(cwp_complex a);
#else
